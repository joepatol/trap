...
pub struct DisplaySerde<T: Serialize>(T);

impl<T: Serialize> Display for DisplaySerde<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<T: Serialize> From<T> for DisplaySerde<T> {
...
mod tests {
    ...
    fn test_lifespan_scope_serialize_ok() {
        let scope = LifespanScope::<String>::new(ASGIScope::default(), Some("state".to_string()));
    ...
    fn test_http_scope_serialize_ok() {
        let scope = HTTPScope::<String>::new(
            ...
            Bytes::new(),
            "".to_string(),
            vec![(Bytes::from("host"), Bytes::from("localhost"))],
            None,
            None,
